...
class hxtool_db:
	...
	def hostsetsList(self):
                with self._lock:
                        return self._db.table('hostsets').all()

        def hostsetsUpdate(self, sets):
                with self._lock:
                        self._db.table('hostsets').purge()
                        self._db.table('hostsets').insert_multiple(sets)
                        self._db.table('hostsets_update').purge()
                        self._db.table('hostsets_update').insert({'update_timestamp' : str(datetime.datetime.utcnow())})
                        
        def hostsetsClear(self):
                with self._lock:
                        self._db.table('hostsets').purge()            
                        
        def hostsetsInsert(self,set_id,set_name,host_id,host_name):
                with self._lock:
                        self._db.table('hostsets').insert({'set_id': set_id,
                                                                   'set_name' : set_name,
                                                                   'host_id' : host_id,
                                                                   'host_name' : host_name})

        def hostsetsGetByHostID(self, host_id):
                with self._lock:
                        return self._db.table('hostsets').search((tinydb.Query()['host_id'] == host_id))

        

                                    
        def hostsetsGetUpdateTimestamp(self):
                with self._lock:
                        timestamp = self._db.table('hostsets_update').all()
                        if len(timestamp) > 0 :
                                return timestamp[0]['update_timestamp']


        def hostsetsSetUpdateTimestamp(self):
                with self._lock:
                        self._db.table('hostsets_update').purge()
                        self._db.table('hostsets_update').insert({'update_timestamp' : str(datetime.datetime.utcnow())})
